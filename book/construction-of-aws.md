...
- Amazon VPCは、AWS上でプライベートクラウドを実現する基盤となるサービスだが、AWSでのVPCの位置付けは多様なITリソースとの関連から**仮想ネットワーク**とされている
- Amazon VPCがまず存在していて、その上にさまざまなITリソースを配備することから、AWSでIaaSを利用する際の必須サービスとして位置づけられている
- AWSを本格的に利用したい企業やオンプレミスのシステムを順次移行していくような企業であれば、企業内ネットワークの一部として、設計した内容に基づいてVPCを作成する
- VPCを1つにするか複数にするか
  - 複数にする典型的な例としては、中規模以上のシステムなどで、本番系と開発計でVPCを分けるケース。ネットワークやITリソースも含めて別物にする
  - 複数の業務システムなどがまったく異なる性質のもので、VPCから分けることがある
- VPCの作成や利用については料金がかからない
- 1つのVPCを細分化する考え方
  - よくある構成は、企業内などのプライベートサブネットと、外部からのアクセスを受け入れるパブリックサブネットでわける
- AWSではCIDR(サイダー)と呼ばれる数値の表記でサブネットを管理している
  - AWSではCIDRはIPアドレスがいくつあるかを、最後の`/`(スラッシュ)以下で、例えば、`/24`=256個、`/16`=65,536個、或いは`/28`=16個のように表す
- IPアドレスの範囲はユーザーが指定できるが、個々のITリソースへのIPアドレスの割当は、DHCP(Dynamic Host Configuration Protocol)の機能によって自動的に行われる
- VPCは始めから存在する(デフォルトVPC)。EC2を作成するとリージョンごとに1つのVPCが存在している。
- NAT(Network Address Translation) ... ネットワークアドレスへの変換機能
  - NATゲートを作成して、プライベートサブネットからインターネットに接続できるようにする
  - プライベートサブネットのEC2インスタンスにはIPアドレスがないため、そのままでは外部にアクセスできず、ソフトウェアのアップデートができない
- IGW(Internet Gateway) ... パブリックIPアドレスと内部のプライベートIPアドレスの紐づけと変換をする
  - IGWを使うことで、プライベートサブネットからパブリックサブネットに変わる
- プライベートサブネットの中から外で出るときのNAT、外から中に入ってくるときのIGW
- TGW(Virtual Private Gateway・仮想プライベートゲートウェイ) ... 専用線接続やVPN接続を介してVPCと外部を接続する
- VPCエンドポイント ... VPCに対応していないサービスにVPCを接続するときに利用される
  - たとえばS3はVPCに載せることができない。しかしながら、S3は使い勝手がよいので、VPCから接続したいという要望はある

## 第7章 RDSとDynamoDBを使う -さまざまなデータベースや分析のサービス-

- データベースを利用するシステムでは、アプリケーション・サーバー(APサーバー)とデータベース・サーバー(DBサーバー)を、それぞれ別サーバーとするのが一般的
- AWSにおいても、データベースを利用する場合には機能に応じて別のインスタンスにする構成が一般的
  - 例えば、アプリケーション専用のEC2インスタンスと、データベース専用のインスタンスに分けて利用する
- Amazon RDS (Amazon Relational Database Service) ... リレーショナルデータベース(RDB)のマネージド・サービス
  - Amazon Aurora, MySQL, PostgreSQL, MariaDB, Oracle, SQL Server を利用できる
- マネージド・サービスのメリットは、ソフトのインストール、アップグレード、バックアップ、スケーリングなどが不要になること
- EC2とは異なり、データベース専用のインスタンスとして、スタンダード、メモリ最適化、バーストの3つから指定できる
- DBエンドポイント ... DBインスタンスに接続するために必須のアドレス
- DBエンドポイントとデータベースポートはRDSの接続に欠かせない
- RDSの利用までの手順
  1. データベースの作成
  2. 接続の準備と接続 ... EC2へのDBクライアント(phpMyAdminなど)のインストール
  3. データの入力と利用
- NoSQL(Not only SQL) ... SQLを使わない。データの急増や多少曖昧な要求でもヒントを得ることができる仕組み

## 第8章 AWSの先進的なサービス -先進技術やトレンドのサービス-

- サーバーレス ... ユーザーがサーバーのことを考えなくてよい、あるいはサーバーを管理しなくていいこと
- AWS Lambda (ラムダ) ... AWSでプログラムを実行する場所としてのサーバーレスサービス
  - プログラム単体で実装・実行できるサービスで、専用の仮想アプリケーション・サーバーが提供されているイメージ
  - DynamoDBやS3と相性がよい
- Amazon SageMaker (セイジメーカー) ... 機械学習のプロセス全体を提供するフルマネージドのサービス
- AWS Amplify ... Firebase とほぼ同じような機能が提供されている

## 第9章 セキュリティと運用管理 -ユーザー、コスト、セキュリティ、運用などの管理-

- AWSでは、セキュリティの基本的な考え方として、責任共有モデルを提唱している
  - クラウドのデータセンターやネットワーク、IT機器などのインフラは、サービス提供者であるAWSの管理責任
  - サービス上で動かすプラットフォームやアプリケーションなどはユーザーが責任を持つ
- ユーザー側のセキュリティで中心となる項目
  1. IDとアクセス管理
  2. ファイアウォール構成
  3. データの暗号化
- IAM用語
  - IAMユーザー ... 組織で1つのAWSアカウントを作成後に、1つ1つのIAMユーザーを使い共用はしない
  - IAMポリシー ... AWSの各サービスに対する操作の許可を定義して、IAMグループにアタッチする
  - IAMグループ ... 所属するIAMユーザーに一様にポリシーを適用できる
  - IAMロール ... AWSのサービスに対しての操作権限を付与するしくみ
- IAMは事前にそれぞれ設計してから権限を付与すべき
